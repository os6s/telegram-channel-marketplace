...
export function WalletConnect({ onWalletConnect }: WalletConnectProps) {
  ...
  const fetchBalance = async (address: string): Promise<string> => {
    try {
      ...
      if (data.ok && data.result.balance) {
        const balanceInTon = (parseInt(data.result.balance) / 1_000_000_000).toFixed(3);
        return balanceInTon;
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (wallet && !connectedOnce) {
      setConnectedOnce(true);

      fetchBalance(wallet.account.address).then((balanceInTon) => {
  ...
  }, [wallet, onWalletConnect, connectedOnce]);
    ...
    return (
      <div className="flex items-center space-x-2">
        <Button
          ...
          onClick={() => window.open(`https://tonapi.io/account/${localWallet.address}`, '_blank')}
        >
...
// Export as EnhancedWalletConnect for backward compatibility
