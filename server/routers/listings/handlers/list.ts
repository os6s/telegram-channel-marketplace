...
import { marketListingsView } from "@shared/schema";
...
export async function listListings(req: Request, res: Response) {
  try {
    ...
    if (search) {
      ...
      conds.push(or(ilike(marketListingsView.title, `%${search}%`), ilike(marketListingsView.username, `%${s}%`)));
    }
    ...
    if (seller && seller.trim() !== "") {
      ...
      // الفيو عندك سترنغ seller (اسم فقط) — نستخدم ilike contains حتى نتفادى "Column seller is required"
